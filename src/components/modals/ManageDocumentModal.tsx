...
function cropImageFile(file: File, image: HTMLImageElement, crop: CropRect, documentTypeKey?: string | null): Promise<File> {
  ...
  const outputWidth = biometric ? MEBBIS_BIOMETRIC_PHOTO_WIDTH : Math.max(1, Math.round(sourceWidth * scale));
  const outputHeight = biometric ? MEBBIS_BIOMETRIC_PHOTO_HEIGHT : Math.max(1, Math.round(sourceHeight * scale));
