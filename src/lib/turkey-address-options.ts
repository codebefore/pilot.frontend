...
const TURKEY_ADDRESS_DISTRICTS_BY_PROVINCE: Record<string, string[]> = {
  "Adana": [
    "Aladağ",
    "Ceyhan",
    "Çukurova",
...
export function resolveTurkeyProvinceValue(value: string | null | undefined): string {
  if (!value) return "";
  const normalized = normalizeAddressValue(value);
  return TURKEY_ADDRESS_PROVINCE_OPTIONS.find((option) => normalizeAddressValue(option.value) === normalized)?.value ?? value.trim();
}

export function resolveTurkeyDistrictValue(
  province: string | null | undefined,
  value: string | null | undefined
): string {
  if (!value) return "";
  const resolvedProvince = resolveTurkeyProvinceValue(province);
  const normalized = normalizeAddressValue(value);
  return TURKEY_ADDRESS_DISTRICTS_BY_PROVINCE[resolvedProvince]?.find(
    (district) => normalizeAddressValue(district) === normalized
  ) ?? value.trim();
}

export function getTurkeyDistrictOptions(
  province: string | null | undefined,
  currentValue?: string
): SelectOption[] {
  const resolvedProvince = resolveTurkeyProvinceValue(province);
  const districts = TURKEY_ADDRESS_DISTRICTS_BY_PROVINCE[resolvedProvince] ?? [];
  const values = [...districts];
  const resolvedCurrent = resolveTurkeyDistrictValue(resolvedProvince, currentValue);
  if (resolvedCurrent && !values.includes(resolvedCurrent)) {
    values.unshift(resolvedCurrent);
  }
  return values.map((district) => ({ value: district, label: district }));
}
